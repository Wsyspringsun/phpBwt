            ...
            foreach ($method as $mValue) {
                ...
                if ($docComment !== false) {
                    ...
                    foreach ($docCommentArr as $comment) {
                        $pos1 = stripos($comment, '@title');
                        if ($pos1 !== false) {
                            $title = substr($comment, $pos1 + 7);
                        }
                        $pos2 = stripos($comment, '@desc');
                        if ($pos2 !== false) {
                            $desc = substr($comment, $pos2 + 5);
                        }
                    }
                }

                $service = $class . '.' . $mValue;
                $serviceShow = $class . '/' . $mValue;      //用于接口显示
                $arrApi[$service] = [
                ...
                <?php
                    ...
                    if (!empty($methods)) {
                        foreach ($methods as $key => $item) {
                            ...
                            echo '<tr>';
                            echo '<td>'.$NO.'</td>';
                            echo '<td><a href="doc.php?f='.$file.'&m='.$item['service'].'">'.$item['serviceShow'].'</a></td>';
